...
class GeneticSolver:
    ...
    def solve(self, ciphertext = ''):
        # Defining ciphertext
        #Allow user to choose How many possible plaintexts to use
        user_choice=int(input("How many possible plaintexts do you wish ? \n"))
        print("Wait a little bit to decrypt the cipher....\n")
        ...
        plaintext = self.convert_to_plaintext(decrypted_text)
        #Printing the chosen number of occurences
        for i in range(0,user_choice):
            print(str(i+1)+' key '+str(key_list[len(key_list)-1-i]))
            print("Decrypted text: \n"+plaintext_list[len(plaintext_list)-1-i]+"\n\n")
